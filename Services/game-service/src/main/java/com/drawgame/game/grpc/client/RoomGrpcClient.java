package com.drawgame.game.grpc.client;

import com.drawgame.room.grpc.generated.*;
import io.grpc.ManagedChannel;
import io.grpc.ManagedChannelBuilder;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.stereotype.Component;

import jakarta.annotation.PostConstruct;
import jakarta.annotation.PreDestroy;

@Slf4j
@Component
public class RoomGrpcClient {

    @Value("${grpc.client.room-service.host:localhost}")
    private String roomServiceHost;

    @Value("${grpc.client.room-service.port:9091}")
    private int roomServicePort;

    private ManagedChannel channel;
    private RoomServiceGrpc.RoomServiceBlockingStub blockingStub;

    @PostConstruct
    public void init() {
        log.info("Connecting to Room Service gRPC at {}:{}", roomServiceHost, roomServicePort);
        this.channel = ManagedChannelBuilder.forAddress(roomServiceHost, roomServicePort)
                .usePlaintext()
                .build();
        this.blockingStub = RoomServiceGrpc.newBlockingStub(channel);
    }

    @PreDestroy
    public void shutdown() {
        if (channel != null && !channel.isShutdown()) {
            channel.shutdown();
        }
    }

    public RoomResponse beginGame(String roomId, String requesterPlayerId) {
        log.info("Calling RoomService.BeginGame for room {}", roomId);
        BeginGameRequest request = BeginGameRequest.newBuilder()
                .setRoomId(roomId)
                .setRequesterPlayerId(requesterPlayerId)
                .build();
        return blockingStub.beginGame(request);
    }

    public RoomResponse getRoom(String roomId) {
        GetRoomRequest request = GetRoomRequest.newBuilder()
                .setRoomId(roomId)
                .build();
        return blockingStub.getRoom(request);
    }

    public RoomResponse finishGame(String roomId) {
        log.info("Calling RoomService.FinishGame for room {}", roomId);
        FinishGameRequest request = FinishGameRequest.newBuilder()
                .setRoomId(roomId)
                .build();
        return blockingStub.finishGame(request);
    }
}
