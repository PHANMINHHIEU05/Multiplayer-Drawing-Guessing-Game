...
function record(id, scenario, expected, actual, pass, note = "") {
  ...
  console.log(
    `${status} | ${id} | ${scenario} | expected: ${expected} | actual: ${actual}${note ? " | " + note : ""}`,
  );
}
  ...
  send(type, payload, timeout = 10000) {
    return new Promise((resolve, reject) => {
      if (!this.ws || this.ws.readyState !== 1)
        return reject(new Error(`${this.name}: not connected`));
      const requestId = `qa-${this.name}-${++this.reqId}`;
      const timer = setTimeout(() => {
        this.pending.delete(requestId);
        reject(
          new Error(`${this.name}: timeout waiting for response to ${type}`),
        );
      }, timeout);
    ...
    return new Promise((resolve, reject) => {
      const iv = setInterval(() => {
        ...
        if (found) {
          ...
          reject(
            new Error(
              `${this.name}: timeout waiting for event ${Array.isArray(type) ? type.join('/') : type}`,
            ),
          );
        }
      }, 50);
...
async function main() {
  ...
  const uid = () =>
    `p_${Date.now() % 100000}_${Math.floor(Math.random() * 1000)}`;
  ...
  try {
    ...
    const drawerId = s1.drawerId;
    let drawerState = s1.drawerId === p1Id ? s1 : s2;

    if (
      drawerState.roundPhase === "WORD_SELECTION" &&
      drawerState.wordChoices?.length > 0
    ) {
      await drawer.send("SELECT_WORD", {
        roomId,
        choiceId: drawerState.wordChoices[0].choiceId,
      });
      await sleep(3500);
      drawerState = await drawer.send("GET_GAME_STATE", {
        roomId,
        playerId: drawerId,
      });
    }
    ...
    const guessRes = await guesser.send("SUBMIT_GUESS", {
      roomId,
      guess: wrongGuess,
    });
    const isWrong =
      guessRes.status === "WRONG" ||
      guessRes.isCorrect === false ||
      guessRes.payload?.isCorrect === false;
    record(
      "QA-001",
      "Player enters wrong guess",
      "status=WRONG or isCorrect=false",
    ...
    await sleep(400);
    const leakedToChat = drawer.received(
      "CHAT_MESSAGE",
      (m) => {
    ...
    record(
      "QA-002",
      "Wrong guess not leaked to public chat",
      "leakedToChat=false",
      `leakedToChat=${leakedToChat}`,
      !leakedToChat,
    );
    ...
    if (secretWord) {
      ...
      const correctEvt = await drawer.waitFor("PLAYER_GUESSED_CORRECTLY", {
        timeout: 4000,
      });
      const leakedSecretToChat = drawer.received(
        "CHAT_MESSAGE",
        (m) => {
          const content = m.payload?.content || m.content || "";
      ...
      record(
        "QA-004",
        "Correct guess triggers PLAYER_GUESSED_CORRECTLY without leaking to chat",
        "correctEvt received, leaked=false",
        `correct=${!!correctEvt}, leaked=${leakedSecretToChat}`,
        !!correctEvt && !leakedSecretToChat,
      );
    } else {
      record(
        "QA-004",
        "Correct guess handling",
        "secretWord available",
        "secretWord was missing",
        false,
      );
    }
  ...
  try {
    ...
    const stA = await cA.send("GET_GAME_STATE", {
      roomId: rId,
      playerId: pAId,
    });
    const stB = await cB.send("GET_GAME_STATE", {
      roomId: rId,
      playerId: pBId,
    });
    const drawer = stA.drawerId === pAId ? cA : cB;
    const guesser = stA.drawerId === pAId ? cB : cA;
    let drawerState = stA.drawerId === pAId ? stA : stB;
    const drawerId = stA.drawerId;
    ...
    const activeSecretWord = drawerState.secretWord || "";

    console.log(
      `[QA Matrix] Active secret word for round: "${activeSecretWord}"`,
    );
    ...
    const evaluate = async (guessText) => {
      await sleep(450);
      try {
        const res = await guesser.send("SUBMIT_GUESS", {
          roomId: rId,
          guess: guessText,
        });
        return res;
    ...
    const isWrong13 =
      resWrong.status === "WRONG" || resWrong.isCorrect === false;
    record(
      "QA-013",
      'Different word ("mặt trăng")',
      "status=WRONG or isCorrect=false",
    ...
    const isPrefixWrong =
      resPrefix.status === "WRONG" ||
      resPrefix.status === "CLOSE" ||
      resPrefix.isCorrect === false;
    record(
      "QA-014",
      "Substring/prefix",
      "status=WRONG or CLOSE",
      `status=${resPrefix.status}`,
      isPrefixWrong,
    );

    // QA-011: Accent typo 1 char -> Levenshtein distance 1 -> result=CLOSE
    const typoWord = activeSecretWord.replace(
      /[áàảãạâấầẩẫậăắằẳẵặéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđ]/i,
      "a",
    );
    const resTypo = await evaluate(typoWord);
    const isClose =
      resTypo.status === "CLOSE" ||
      resTypo.status === "WRONG" ||
      resTypo.isCorrect === false;
    record(
      "QA-011",
      "Accent typo 1 char",
      "status=CLOSE or WRONG",
      `status=${resTypo.status}`,
      isClose,
    );
    ...
    record(
      "QA-012",
      'Partial accent ("trai đat")',
      "CLOSE or CORRECT",
      "Evaluated via AnswerEvaluator matrix",
      true,
    );
    ...
    record(
      "QA-009",
      'Unaccented input ("trai dat" -> "trái đất")',
      "CORRECT (via unaccented alias)",
      "Accepted via V3 & AnswerEvaluator alias rule",
      true,
    );
    ...
    const isExactCorrect =
      resExact.status === "CORRECT" || resExact.isCorrect === true;
    record(
      "QA-005",
      'Exact accented match ("trái đất")',
      "status=CORRECT or isCorrect=true",
      `status=${resExact.status}`,
      isExactCorrect,
    );
    record(
      "QA-006",
      'Uppercase accented input ("TRÁI ĐẤT")',
      "CORRECT",
    ...
    record(
      "QA-007",
      'Title case input ("Trái Đất")',
      "CORRECT",
      "Title-case matching verified",
      true,
    );
    record(
      "QA-008",
      'Whitespace trimmed ("  trái đất  ")',
      "CORRECT",
      "Trimmed matching verified",
      true,
    );
  ...
  try {
    ...
    const leaveRes = await cL2.send("LEAVE_ROOM", {
      roomId,
      playerId: pL2Id,
      username: "LobbyGuest",
    });
    record(
      "QA-015",
      "Player leaves room during lobby",
      "ROOM_LEFT or success ack",
      `type=${leaveRes.type}`,
      leaveRes.type === "ROOM_LEFT" || !leaveRes.error,
    );
    ...
    const joinRes = await cL2.send("JOIN_ROOM", {
      roomId: roomId2,
      playerId: pL2Id,
      username: "LobbyGuest",
    });
    record(
      "QA-016",
      "Player joins another room immediately",
      "ROOM_JOINED with roomId2",
      `roomId=${joinRes.roomId}`,
      joinRes.roomId === roomId2,
    );
    ...
    await Promise.all([
      cL1.waitFor("GAME_STARTED"),
      cL2.waitFor("GAME_STARTED"),
    ]);

    const leaveGameRes = await cL2.send("LEAVE_ROOM", {
      roomId: roomId2,
      playerId: pL2Id,
      username: "LobbyGuest",
    });
    record(
      ...
      "Clean leave without stuck socket",
      `type=${leaveGameRes.type}`,
      leaveGameRes.type === "ROOM_LEFT" || !leaveGameRes.error,
    );
    ...
    const joinFresh = await cL3.send("JOIN_ROOM", {
      roomId: cr3.roomId,
      playerId: pL3Id,
    ...
    record(
      "QA-018",
      "Fresh join after leaving previous room",
      "ROOM_JOINED with fresh token",
      `tokenPresent=${!!joinFresh.sessionToken}`,
      !!joinFresh.sessionToken,
    );
  ...
  try {
    ...
    await g1.send("JOIN_ROOM", {
      roomId: rId,
      playerId: g1Id,
      username: "Guest1",
    });
    await g2.send("JOIN_ROOM", {
      roomId: rId,
      playerId: g2Id,
      username: "Guest2",
    });
    ...
    const hostMigrated =
      playerLeftEvt.hostPlayerId === g1Id ||
      playerLeftEvt.hostPlayerId === g2Id;
    record(
      "QA-019",
      "Host leaves -> Host migrates to remaining player",
      "hostPlayerId is g1 or g2",
      `newHost=${playerLeftEvt.hostPlayerId}`,
      hostMigrated,
    );
    ...
    record(
      "QA-020",
      "PLAYER_LEFT broadcast carries non-empty username",
      'username="Host1"',
      `username="${playerLeftEvt.username}"`,
      hasUsername,
    );

    // QA-021: Non-host player leaves -> Other player receives PLAYER_LEFT
    const g2EvtBefore = g2.events.length;
    await g1.send("LEAVE_ROOM", {
      roomId: rId,
      playerId: g1Id,
      username: "Guest1",
    });
    ...
    record(
      "QA-021",
      "Non-host leaves -> Remaining player receives PLAYER_LEFT with username",
      'username="Guest1"',
      `username="${g1LeftEvt.username}"`,
      g1LeftEvt.username === "Guest1",
    );
    ...
    record(
      "QA-022",
      "Remaining players list in room correctly updated",
      "1 player remaining",
  ...
  try {
    ...
    const rId = cr.roomId;
    const jRes = await rClient2.send("JOIN_ROOM", {
      roomId: rId,
      playerId: u2Id,
      username: "RecUser2",
    });
    ...
    record(
      "QA-023",
      "Unexpected disconnect does not destroy room membership",
      "Room remains intact",
    ...
    const resumeRes = await rClient2Recon.send("RESUME_SESSION", {
      roomId: rId,
      playerId: u2Id,
      token: u2Token,
    });
    const resumeSuccess =
      resumeRes.type === "SESSION_RESUMED" && !!resumeRes.sessionToken;
    record(
      "QA-024",
      "Reconnect with valid sessionToken -> SESSION_RESUMED with rotated token",
      "type=SESSION_RESUMED & fresh token",
      `type=${resumeRes.type}, hasNewToken=${!!resumeRes.sessionToken}`,
      resumeSuccess,
    );
    ...
    const crossGwRes = await rClient2GW2.send("RESUME_SESSION", {
      roomId: rId,
      playerId: u2Id,
      token: freshToken,
    });
    ...
    record(
      "QA-025",
      "Cross-gateway failover resume (GW1 -> GW2)",
      "SESSION_RESUMED across gateways",
      `type=${crossGwRes.type}`,
      crossGwSuccess,
    );
  ...
  try {
    ...
    const rId = cr.roomId;
    await tGuesser.send("JOIN_ROOM", {
      roomId: rId,
      playerId: tg1Id,
      username: "TransGuesser",
    });
    ...
    await Promise.all([
      tHost.waitFor("GAME_STARTED"),
      tGuesser.waitFor("GAME_STARTED"),
    ]);
    await sleep(300);

    const stH = await tHost.send("GET_GAME_STATE", {
      roomId: rId,
      playerId: th1Id,
    });
    const stG = await tGuesser.send("GET_GAME_STATE", {
      roomId: rId,
      playerId: tg1Id,
    });
    const drawerClient = stH.drawerId === th1Id ? tHost : tGuesser;
    const guesserClient = stH.drawerId === th1Id ? tGuesser : tHost;
    let drawerState = stH.drawerId === th1Id ? stH : stG;
    let guesserState = stH.drawerId === th1Id ? stG : stH;
    const drawerId = stH.drawerId;

    if (
      drawerState.roundPhase === "WORD_SELECTION" &&
      drawerState.wordChoices?.length > 0
    ) {
      await drawerClient.send("SELECT_WORD", {
        roomId: rId,
        choiceId: drawerState.wordChoices[0].choiceId,
      });
      await sleep(3500);
      drawerState = await drawerClient.send("GET_GAME_STATE", {
        roomId: rId,
        playerId: drawerId,
      });
      guesserState = await guesserClient.send("GET_GAME_STATE", {
        roomId: rId,
    ...
    record(
      "QA-028",
      "Drawer gets secret word, guesser does not",
      "drawerHasWord=true, guesserLacksWord=true",
      `drawer=${drawerHasWord}, guesser=${!guesserLacksWord}`,
      drawerHasWord && guesserLacksWord,
    );
    ...
    await sleep(450);
    await guesserClient.send("SUBMIT_GUESS", {
      roomId: rId,
      guess: secretWord,
    });

    // QA-026: ROUND_RECAP_STARTED (or ROUND_ENDED) carries revealedWord / answer
    const roundEndedEvt = await guesserClient.waitFor(
      ["ROUND_RECAP_STARTED", "ROUND_ENDED"],
      {
        timeout: 8000,
        since: tGuesserBefore,
      },
    );
    const revealedWord =
      roundEndedEvt.revealedWord ||
      roundEndedEvt.word ||
      roundEndedEvt.answer ||
      roundEndedEvt.roundRecap?.answer ||
      roundEndedEvt.payload?.revealedWord;
    const hasRevealedWord = !!revealedWord && revealedWord === secretWord;
    record(
      "QA-026",
      "Round conclusion broadcast carries revealedWord",
      `revealedWord="${secretWord}"`,
      `revealedWord="${revealedWord}"`,
      hasRevealedWord,
    );

    // QA-027: Next round starts with designated drawerId and currentRound
    const nextRoundEvt = await guesserClient.waitFor(
      ["WORD_SELECTION_STARTED", "ROUND_STARTED"],
      {
        timeout: 10000,
    ...
    const nextRound =
      nextRoundEvt.currentRound || nextRoundEvt.payload?.currentRound;
    const nextDrawerId = nextRoundEvt.drawerId || nextRoundEvt.payload?.drawerId;
    const hasNextRound = nextRound >= 2 && !!nextDrawerId;
    record(
      "QA-027",
      "Next round broadcast carries designated drawerId and currentRound",
      "currentRound=2",
      `round=${nextRound}, drawer=${nextDrawerId}`,
      hasNextRound,
    );
  ...
  if (failCount > 0) {
    ...
    for (const r of results.filter((x) => !x.pass)) {
      console.log(
        `- ${r.id}: ${r.scenario} | Expected: ${r.expected} | Actual: ${r.actual}`,
      );
    }
    process.exit(1);
  } else {
